<?php
			...
			if(count($Err) == 0){
				...
				if($num_rows > 0){
					...
					while($row = mysqli_fetch_array($seat_status_r,MYSQLI_ASSOC)){
						$movie_name = $row['seat_id'];
						$message['booked_seat'][$row['seat_id']] = $row['seat_id'];
					}
				...
				$booking_info_q = "insert into booking_info(name,email,mobile,booking_date,movie_id,show_timing_id,status) values('$name','$email','$mobile',now(),'$movie_id','$show_id','1')";
				$booking_info_r = mysqli_query($con,$booking_info_q);
				$booking_id = mysqli_insert_id($con);
				
				foreach($tickets as $value){
					$ticket_detail_q .="('$booking_id','$value'),"; 
				}
	...
	function send_mail($to,$subject, $body){
		...
		$from = new SendGrid\Email(null, "abhay.mlvtec@gmail.com");
		$subject = $subject;
                                    ...
                                    <tbody>
                                       <tr>
                                          <td valign='top' align='left' style='width:60px;height:40px;padding:40px 30px 0px 30px;vertical-align:middle;text-align:center;font-size:26px;font-weight:800;font-family:Arial,sans-serif;color:#000000;border:0'>
                                             <p style='margin:0;padding-bottom:6px;line-height:0;border:0'>$qty</p>
